...
# # Hippocampus Dataset.
# dir_img = Path('/home/mail/2020q0/q030026150/dataset/Hippocampus/imagesTr/')
# dir_mask = Path('/home/mail/2020q0/q030026150/dataset/Hippocampus/labelsTr/')

# Pancreas Dataset.
# dir_img = Path('/home/mail/2020q0/q030026150/dataset/Pancreas/imagesTr')
# dir_mask = Path('/home/mail/2020q0/q030026150/dataset/Pancreas/labelsTr')

# BraTS Dataset.
# dir_brats = Path('./MICCAI_BraTS2020_TrainingData')
dir_brats = Path('/root/autodl-tmp/MICCAI_BraTS2020_TrainingData')
...
def train_model(model, device, epochs: int = 20, batch_size: int = 16, learning_rate: float = 1e-5, val_percent: float = 0.5, val_step: float = 10, 
    ...
    try:
        if datasets == 'Brats':
            ...
            train_list = [transforms.Resize((input_size, input_size), antialias=True)]
            transform_train = transforms.Compose(train_list)
            dataset = HIPPODataset3D(dir_img, dir_mask, transform_train, test_flag=False)
        elif datasets == 'ISIC':
            # args = create_argparser().parse_args()
            train_list = [transforms.Resize((input_size, input_size)), transforms.ToTensor(),]
            transform_train = transforms.Compose(train_list)
            dataset = ISICDataset(dir_img, dir_mask, transform_train)
        else:
    ...
    for epoch in range(1, epochs + 1):
        ...
        if validation_step(epoch, val_step) == True: # be adviced epoch started at 1
            ...
            # print("here")
            # print("val l: ", len(val_score))
            # print("val: ", val_score)
            # val_score_str = str(val_score)
            # print("val: ", val_score_str)
