...
# Bind contextual logger for this module. You can include `{extra[module]}` in your
# Loguru sink format to display this tag, or use it to filter logs from this module.
log = _base_logger.bind(module="bratsloader")
...
class BRATSDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, directory, transform=None, test_flag=False):
        ...
        log.info(f"BRATSDataset initialized with {len(self.database)} volumes")
        ...
        for root, dirs, files in os.walk(self.directory):
            ...
            if not dirs:
                ...
                if set(datapoint.keys()) == self.seqtypes_set:
                    ...
                    log.warning(f"Incomplete datapoint in {root}")
                    if missing:
                        log.warning(f"  Missing: {missing}")
                    if extra:
                        log.warning(f"  Extra: {extra}")
        
        if not database:
            log.error(f"No valid datapoints found in {self.directory}")
...
class BRATSDataset3D(torch.utils.data.Dataset):
    ...
    def __init__(self, directory, transform=None, test_flag=False):
        ...
        log.info(f"BRATSDataset3D initialized with {len(self.database)} volumes, {len(self)} total slices")
