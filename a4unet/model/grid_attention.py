...
class _GridAttentionBlockND(nn.Module):
    def __init__(self, in_channels, emb_channels, gating_channels, inter_channels=None, dimension=2, mode='concatenation',
    ...
    # _concatenation_debug模式使用Softplus激活函数，可能会导致较大的梯度，因此在一些情况下可能不太稳定
    # _concatenation_residual模式使用了Softmax，通常更适用于需要归一化权重的情况，但不太适合在2D图像上进行处理。
...
class GridAttentionBlock2D(_GridAttentionBlockND):
    def __init__(self, in_channels, emb_channels, gating_channels, inter_channels=None, mode='concatenation',
                 sub_sample_factor=(2, 2)):
